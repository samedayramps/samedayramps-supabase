...
export async function createSetupFeeInvoice(jobId: string) {
  ...
  if (job.setup_fee <= 0) {
    throw new Error("Setup fee must be greater than 0");
  }
  ...
  try {
    ...
    let setupFeeInvoice = await stripe.invoices.create({
    ...
    return {
      setupFeeInvoiceUrl: setupFeeInvoice.hosted_invoice_url,
      success: true
    };
  } catch (error) {
...
export async function createMonthlyInvoice(jobId: string) {
  const user = await getCurrentUser();
  if (!user) throw new Error("Not authenticated");
  await requireRole(user.id, "admin");
  ...
  try {
    ...
    return {
      monthlyInvoiceUrl: finalizedInvoice.hosted_invoice_url,
